...
use crate::pages::{EditCollectionPage, UpdatePage};
use image_dl::StatusUpdate;
use ygod_core::{
    ext_data::image_dl,
...
impl Window {
    ...
    pub fn update(&self, finished_sender: glib::Sender<()>) {
        ...
        let progress_updater = ProgressUpdater::new(progress_sender);
...
struct ProgressUpdater {
    progress_sender: glib::Sender<(f64, String)>,
}

impl StatusUpdate for ProgressUpdater {
    fn update(&self, fraction: f64, text: String) {
        self.progress_sender.send((fraction, text)).expect("Could not send through channel.");
    }
}

impl ProgressUpdater {
    pub fn new(progress_sender: glib::Sender<(f64, String)>) -> Self {
